...
if [ "$MODE" == "fix" ]; then
    git ls-files | grep -e '\.ts$' -e '\.js$' -e '\.md$' -e '\.yaml$' -e '\.yml$' -e '\.json$' | grep -v 'gen/.*' | grep -v 'governance/proposals/.*' | xargs npx prettier --write
else
    git ls-files | grep -e '\.ts$' -e '\.js$' -e '\.md$' -e '\.yaml$' -e '\.yml$' -e '\.json$' | grep -v 'gen/.*' | grep -v 'governance/proposals/.*' | xargs npx prettier --check
fi
